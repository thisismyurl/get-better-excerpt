...
WordPress core truncates excerpts by character count, which can break a word in the middle. This plugin truncates on a word boundary so excerpts read cleanly.

## Features

- Word-boundary excerpt truncation.
- The same API surface as `get_the_excerpt()` and `the_excerpt()`.
